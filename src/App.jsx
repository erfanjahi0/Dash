...
function LiveBackground({ data }) {
  return (
    <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
      <motion.div
        className="absolute -left-24 top-20 h-72 w-72 rounded-full blur-3xl"
        style={{ backgroundColor: data.accentColor, opacity: 0.16 }}
        animate={{ x: [0, 36, 0], y: [0, -24, 0], scale: [1, 1.12, 1] }}
        transition={{ duration: 9, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute -right-28 top-72 h-80 w-80 rounded-full blur-3xl"
        style={{ backgroundColor: data.primaryColor, opacity: 0.12 }}
        animate={{ x: [0, -42, 0], y: [0, 30, 0], scale: [1, 1.16, 1] }}
        transition={{ duration: 11, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute bottom-20 left-1/2 h-56 w-56 rounded-full blur-3xl"
        style={{ backgroundColor: data.primaryColor, opacity: 0.08 }}
...
function LiveActivityCard({ data }) {
  const bars = [32, 58, 44, 72, 38, 66, 48];
  return (
    <div className="mt-6 rounded-3xl border border-white/70 bg-white/80 p-4 shadow-sm backdrop-blur-xl">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-500">Auto-refresh feel</span>
      </div>
      <div className="flex h-16 items-end gap-2">
        {bars.map((height, index) => (
          <motion.span
            ...
            style={{ background: `linear-gradient(180deg, ${data.accentColor}, ${data.primaryColor})` }}
            animate={{ height: [`${height}%`, `${Math.min(height + 24, 96)}%`, `${height}%`] }}
            transition={{ duration: 1.5 + index * 0.12, repeat: Infinity, ease: "easeInOut", delay: index * 0.08 }}
          />
        ))}
      </div>
    </div>
...
function PublicView({ data, setMode }) {
  ...
  return (
    <div style={{ backgroundColor: data.backgroundColor, color: data.textColor }} className="relative min-h-screen overflow-hidden">
      <LiveBackground data={data} />
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="flex items-center gap-1.5 text-xs text-slate-500"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />Live project status</p>
            </div>
          </div>
          <button
